...
sub create_player
{
    ...
    $log_name =~ s/ /_/g;
    my $pl = BalanceOfPower::Player->new(name => $username, money => $money, log_name => $log_name, log_dir => $self->log_dir, current_year => $self->current_year, position => $position, log_active => $self->log_active);
    $pl->delete_log();
