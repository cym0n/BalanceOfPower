...
sub newspaper {
    ...
    {
        my @data = ();
        my $cursor = $c->db('bop_events')->get_collection($game)->find({ code => $event, 'time' => "$year/$turn", "source_type" => 'world'});
    ...
    my @warevents = ( $c->db('bop_events')->get_collection($game)->find({ code => 'warstart', 'time' => "$year/$turn", "source_type" => 'world'})->all);
    @warevents = (@warevents, $c->db('bop_events')->get_collection($game)->find({ code => 'warlinkedstart', 'time' => "$year/$turn", "source_type" => 'world'})->all);
    @warevents = (@warevents, $c->db('bop_events')->get_collection($game)->find({ code => 'warend', 'time' => "$year/$turn", "source_type" => 'world'})->all);
    my %wars;
